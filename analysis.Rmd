...
p <- ggplot(res, aes(x = frac, y = plot_feature, color = plot_feature_value)) +
    ...
    theme_bw() +
    labs(x = "Fraction who respond \"Yes\"", y = "Plot feature") +
...
print(xtable(feature_freqs), include.rownames = FALSE)
```
...
summ_plottypes_base <- summ_plottypes_base %>% select(plot_type, results_base = results)
summ_plottypes_gg <- summ_plottypes_gg %>% select(plot_type, results_gg = results)

df_clarity_plot_types <- full_join(summ_plottypes_base, summ_plottypes_gg)
colnames(df_clarity_plot_types) <- c("Plot type", "Base R", "ggplot2")
